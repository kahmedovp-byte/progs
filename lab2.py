...
def parse_meter_reading(line: str) -> MeterReading:
    resource = re.search(r'"([^"]+)"', line).group(1)
...
class MeterApp:
    ...
    def add_reading(self):
        ...
        tk.Label(form, text="Дата (гггг-мм-дд):").grid(row=1, column=0)
        date_entry = tk.Entry(form)
        date_entry.grid(row=1, column=1)

        tk.Label(form, text="Значение:").grid(row=2, column=0)
        ...
        def save():
            ...
            new_reading = MeterReading(resource_entry.get(),
                                       reading_date,
                                       value)
        ...
        tk.Button(form, text="Сохранить",
                  command=save).grid(row=3, column=0, columnspan=2)
